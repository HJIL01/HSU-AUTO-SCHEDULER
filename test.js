const cs = require("./courses");

const test = cs.map((c) => ({
  ...c,
  classTimeAndRoom: formatClassInfo(
    c.deliveryMethod,
    c.credit,
    c.classTimeAndRoom
  ),
...
test.map((e) => console.log(e.classTimeAndRoom));

/* 
      요일과 시간을 분리하여 배열 형태로 return 하는 함수
...
*/
function getDayAndTime(dayAndTime) {
...
*/
function extractInPersonSchedule(inPersonSchedule) {
  ...
  return formatted.map((schedule) => ({
    place: schedule[0],
    day: schedule[1],
    startTime: Number(schedule[2]) * 9 * 60,
    endTime: Number(schedule[3]) * 9 * 60,
  }));
}

function formatClassInfo(deliveryMethod, credit, classRoom) {
