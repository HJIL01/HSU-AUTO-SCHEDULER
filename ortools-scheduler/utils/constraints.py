...
def set_objective_maximize_credit(courses: CourseSchema, model, selected):
    # 최대 학점 지향하는 목적식
    # 이게 없다면 0학점도 나올 수 있음
...
def add_max_credit_constraint(courses: CourseSchema, model, selected, max_credit: int):
    # 최대 학점 제한
    limit_max = (
        sum(course.credit * cur for course, cur in zip(courses, selected)) == max_credit
    )
...
def add_deduplicated_course_constraint(courses: CourseSchema, model, selected):
...
def add_major_foundation_min_constraint(
    courses: CourseSchema, model, selected, major_foundation_credit
):
    model.Add(
        sum(
            course.credit * cur
...
def add_major_required_min_constraint(
    courses: CourseSchema, model, selected, major_required_credit
):
    model.Add(
        sum(
            course.credit * cur
...
def add_major_elective_min_constraint(
    courses: CourseSchema, model, selected, major_elective_credit
):
    model.Add(
        sum(
            course.credit * cur
...
def add_daily_lecture_limit_constraint(
    courses: CourseSchema, model, selected, daily_lecture_limit
):
...
# 연강을 지향할 제약조건 추가 함수
def add_class_gap_constraint(
    courses: CourseSchema, model, is_selected, allowed_gap_mins
):
    course_day_indices = defaultdict(list)
